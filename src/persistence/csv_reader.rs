...
use crate::models::{export_record::ExportRecord, natural_gas_liquid_export::{NaturalGasLiquidExport, NATURALGASLIQUID_HEADERS}};
        ...
        match result {
            Ok(record) => {
                ...
                if record.len() == NATURALGASLIQUID_HEADERS.len() {
                    let export =  Box::new(NaturalGasLiquidExport::new(
                        record[0].to_string(),
                        record[1].parse::<u16>()?, 
                        record[2].to_string(),
                        record[3].to_string(),
